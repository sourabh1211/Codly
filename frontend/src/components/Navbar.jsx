...
const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-20 h-[80px] bg-transparent backdrop-blur-md border-b border-gray-700/30 shadow-[0_2px_10px_rgba(0,0,0,0.1)]">
      <img
        src={logo}
        alt="Logo"
        className="w-[140px] md:w-[160px] object-contain cursor-pointer transition duration-300 hover:opacity-90 hover:scale-105"
      />

      <div className="flex items-center gap-4 md:gap-8 text-white text-[15px] md:text-[16px] font-medium">
        {Object.keys(routes).map((item, index) => (
          <Link
            to={routes[item]}
            key={index}
            className="relative px-2 py-1 group transition-all duration-300 text-white hover:text-cyan-400"
          >
            {item}
            <span className="absolute left-0 bottom-0 w-0 h-[2px] bg-cyan-400 transition-all duration-300 group-hover:w-full"></span>
          </Link>
        ...
        <button
          ...
          className="ml-2 px-5 py-2 rounded-full font-semibold text-white bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 bg-clip-text text-transparent shadow-md hover:shadow-yellow-400/30 hover:scale-105 transition-all duration-300"
        >
          Logout
        </button>
      </div>
