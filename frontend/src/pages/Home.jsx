...
const Home = () => {
  ...
  return (
    <>
      <Navbar />
      <div className="flex items-center justify-between px-8 md:px-16 mt-8">
        <h3 className="relative text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-blue-400 to-purple-500 animate-text-lg">
          👋 Welcome To Codly
        </h3>
        <button
          onClick={() => setIsCreateModelShow(true)}
      ...
      <div className="projects grid grid-cols-1 md:grid-cols-2 gap-6 px-8 md:px-16 mt-8 pb-10">
        {projects && projects.length > 0 ? (
          projects.map((project, idx) => (
            ...
            >
              <div className="flex items-center gap-5">
                ...
                />
                <div>
                  <h4 className="text-xl font-bold text-white drop-shadow-lg">{project.name}</h4>
                  <p className="text-sm text-gray-400">{new Date(project.date).toDateString()}</p>
                </div>
              </div>
          ...
          <p className="col-span-2 text-center text-gray-500 text-lg">🚫 No Project Found!</p>
        )}
      </div>
      ...
      {isCreateModelShow && (
        ...
        >
          <div className="bg-black bg-opacity-60 p-8 rounded-3xl w-11/12 md:w-1/3 border border-gray-600 shadow-xl backdrop-filter backdrop-blur-lg">
            ...
            {selectedLanguage && (
              <>
                <p className="mt-3 text-green-400">Selected: {selectedLanguage.label}</p>
                <button onClick={createProj} className="mt-4 w-full py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 hover:scale-105 transition transform">Create</button>
              </>
            )}
          </div>
        </div>
