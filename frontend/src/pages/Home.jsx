...
//       color: '#fff',
//       cursor: 'pointer',
//       padding: '12px 12px',      // better tap size
...
//                     <p className="text-xs sm:text-sm text-slate-400">{new Date(project.date).toDateString()}</p>
//                   </div>
//                 </div>
  ...
  return (
    <>
      ...
      <main className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800">
        ...
        {/* Header (colors aligned to About’s cyan→blue accent) */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-4 sm:px-8 md:px-16 pt-8 md:pt-10">
          <h3 className="text-2xl sm:text-3xl font-bold text-white relative group transition-all duration-300">
            👋{" "}
            <span className="inline-block bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">
        ...
        {/* Projects grid (glass cards + same borders/text as About) */}
        <div className="px-4 sm:px-8 md:px-16 mt-6 md:mt-8 pb-16 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-6">
          {projects && projects.length > 0 ? (
            projects.map((project, index) => (
              <div
                ...
                className="group relative overflow-hidden p-5 rounded-2xl
                           ...
                           touch-pan-y"
              >
                {/* cursor spotlight (indigo-tinted like About) */}
                <div
                  className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  style={{
                    background:
